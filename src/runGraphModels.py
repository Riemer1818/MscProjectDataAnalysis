...
class GATWithResidual(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=1, heads=1, dropout=0.5):
        super(GATWithResidual, self).__init__()
        self.convs = torch.nn.ModuleList()
        self.dropout = dropout
...
class GIN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=1):
        ...
        # Define GIN layers and batch norm layers
...
# =========================
def train_and_evaluate_model(model, dataloader, val_dataloader, num_epochs, learning_rate, T_max, device, checkpoint_dir):
    ...
    checkpoint_dir = os.path.join(checkpoint_dir, model.__class__.__name__)
...
print("Training GAT with residual connections model...")
gat_residual_model = GATWithResidual(in_channels, hidden_channels, out_channels, num_layers=num_layers, heads=4, dropout=0.5)
train_losses, val_losses = train_and_evaluate_model(
    model=gat_residual_model,
    dataloader=dataloader,
    val_dataloader=val_dataloader,
    num_epochs=num_epochs,
    learning_rate=learning_rate,
